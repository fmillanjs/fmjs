...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  5. Test RBAC enforcement directly via curl:

     a) POST /workspaces without a token returns 401:
        ```
        curl -s -o /dev/null -w "%{http_code}" -X POST http://localhost:3003/workspaces \
          -H "Content-Type: application/json" \
          -d '{"name":"test"}'
        ```
        Expected: 401

     b) POST /workspaces/:slug/invite-links as a Contributor (not Admin) returns 403:
        ```
        # Signup a second user (will be invited as Contributor role)
        curl -c /tmp/contrib.cookie http://localhost:3003/auth/signup \
          -H "Content-Type: application/json" \
          -d '{"email":"contrib@test.com","password":"Pass123!","name":"Contrib User"}'

        # As Admin: generate an invite link (default role Contributor)
        # Get admin cookie first by logging in, then:
        curl -b <admin-cookie> -X POST http://localhost:3003/workspaces/my-team/invite-links
        # Note the token

        # Contributor joins workspace
        curl -b /tmp/contrib.cookie -X POST http://localhost:3003/workspaces/join \
          -H "Content-Type: application/json" \
          -d '{"token":"<token>"}'

        # Contributor tries to create an invite link — must get 403
        curl -s -o /dev/null -w "%{http_code}" -b /tmp/contrib.cookie \
          -X POST http://localhost:3003/workspaces/my-team/invite-links
        ```
        Expected: 403

     c) DELETE /workspaces/:slug/members/:userId removing the last Admin returns 400:
        ```
        # Get your userId from the workspace members list
        curl -b <admin-cookie> http://localhost:3003/workspaces/my-team

        # Attempt to remove yourself as the only Admin
        curl -s -o /dev/null -w "%{http_code}" -b <admin-cookie> \
          -X DELETE http://localhost:3003/workspaces/my-team/members/<your-userId>
        ```
        Expected: 400 Bad Request ("Cannot remove the last Admin from a workspace")

  Note: Viewer 403 on snippet/post endpoints is NOT tested here — those endpoints
  arrive in Phase 17. Phase 16 proves the CASL guard infrastructure is wired and working.
  </how-to-verify>
...
- Human verification: workspace creation, member join, unauthenticated 401, Contributor 403 on invite-link, last-admin 400 all confirmed
</success_criteria>
