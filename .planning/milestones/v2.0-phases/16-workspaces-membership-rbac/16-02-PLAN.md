...
# RBAC-02 and RBAC-03 (Contributor edit-own / Viewer read-only on snippets/posts) are
# intentionally deferred to Phase 17. Those requirements require snippet and post API
# endpoints (and the authorId field on Prisma models) to be demonstrable. Phase 16 proves
# the CASL guard infrastructure is wired and working; snippet/post enforcement follows in
# Phase 17 when those endpoints exist.

must_haves:
  truths:
    - "An Admin user on a workspace-scoped endpoint receives 200 for manage operations"
    - "A Viewer user attempting a create operation on a workspace-scoped endpoint receives 403 ForbiddenException"
    - "A Contributor user receives 403 for manage-workspace operations (e.g. POST invite-links) and 200 for permitted actions"
    ...
    # NOTE — field-level conditions (RBAC-02/03): WorkspaceAbilityFactory grants Contributor
    # unconditional update ability on Post/Snippet/Comment with no { authorId: userId } condition.
    # The 'own content only' constraint is deferred to Phase 17 when authorId is present on
    # Prisma models and the snippet/post endpoints exist to exercise it.
  artifacts:
...
- RBAC-01 (guard enforces CASL rules for workspace operations) proven by Phase 16 workspace endpoints
- RBAC-04 (deny-by-default) proven by meta-test and guard ForbiddenException path
- RBAC-02/03 (Contributor own-content-only, Viewer read-only on snippets/posts) deferred to Phase 17
</success_criteria>
