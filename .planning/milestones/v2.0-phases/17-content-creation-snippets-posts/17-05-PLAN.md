...
must_haves:
  truths:
    ...
    - "Contributor JWT (default role on join) gets 201 on POST /workspaces/:slug/snippets"
  artifacts:
...
RBAC-02 specifically requires a Contributor-role user (not Admin) to succeed with 201. The workspace creator is always Admin. A third user must join the workspace and keep the default Contributor role — that user's session is used for the RBAC-02 assertion.
...
<task type="auto">
  ...
  <files>
    <!-- Validation-only task: no source files are created or modified.
         This task runs shell commands to build and smoke-test the live system. -->
  </files>
  <action>
...
**Step 3 — RBAC smoke tests:**

Three users are needed:
- **admin-test** — creates the workspace (becomes Admin automatically)
- **contributor-test** — joins the workspace and keeps the default Contributor role (DO NOT promote to Admin)
- **viewer-test** — joins the workspace and is then demoted to Viewer role

```bash
# Register admin user (workspace creator)
curl -c /tmp/admin-cookie.txt -X POST http://localhost:3003/auth/signup \
  -H "Content-Type: application/json" \
  -d '{"email":"admin-test@example.com","name":"Admin Test","password":"Test1234!"}'

# Register contributor user (will keep default Contributor role)
...
# Register viewer user (will be demoted)
...
```bash
curl -b /tmp/admin-cookie.txt -X POST http://localhost:3003/workspaces \
  -H "Content-Type: application/json" \
  -d '{"name":"RBAC Test WS","slug":"rbac-test-ws"}'
```

Generate an invite link (as admin) and have both contributor and viewer join:
```bash
INVITE=$(curl -s -b /tmp/admin-cookie.txt -X POST http://localhost:3003/workspaces/rbac-test-ws/invite-links)
TOKEN=$(echo $INVITE | grep -o '"token":"[^"]*"' | cut -d'"' -f4)

# Contributor joins — default role on join is Contributor, do NOT change it
curl -b /tmp/contributor-cookie.txt -X POST http://localhost:3003/workspaces/join \
  -H "Content-Type: application/json" \
  -d "{\"token\":\"$TOKEN\"}"
...
Demote viewer-test to Viewer role (admin performs the role change):
```bash
VIEWER_ID=$(curl -s -b /tmp/viewer-cookie.txt http://localhost:3003/auth/me | grep -o '"sub":"[^"]*"' | cut -d'"' -f4)
curl -b /tmp/admin-cookie.txt -X PATCH \
  http://localhost:3003/workspaces/rbac-test-ws/members/$VIEWER_ID/role \
  -H "Content-Type: application/json" \
  -d '{"role":"Viewer"}'
```

NOTE: contributor-test is NOT promoted — it keeps whatever role the API assigns on join (expected: Contributor). Verify contributor's current role before running the RBAC-02 test:
```bash
curl -s -b /tmp/admin-cookie.txt http://localhost:3003/workspaces/rbac-test-ws/members | \
  grep -A2 "contributor-test"
```

Now run RBAC assertions:
```bash
...
# RBAC-02: Contributor (contributor-test, default role, NOT Admin) should get 201
CONTRIBUTOR_SNIPPET=$(curl -s -o /dev/null -w "%{http_code}" \
  -b /tmp/contributor-cookie.txt \
  -X POST http://localhost:3003/workspaces/rbac-test-ws/snippets \
  -H "Content-Type: application/json" \
  -d '{"title":"Contributor Snippet","language":"typescript","code":"const x = 1"}')
echo "Contributor create snippet: $CONTRIBUTOR_SNIPPET (expect 201)"
```

If RBAC-02 returns 403 for the Contributor, that is a bug in the CASL guard. Diagnose by checking the member's role in the DB and the CASL ability definition. Fix before proceeding to checkpoint.

Record all results. If any test fails, diagnose and fix before proceeding.
  </action>
  <verify>
...
5. Contributor create snippet returns 201 (Contributor role, not Admin)
  </verify>
  <done>Production build succeeds. RBAC smoke tests: Viewer=403 on both endpoints, Contributor (default join role, not Admin)=201 on snippet creation. Results logged for human verification.</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 2: Human verify Phase 17 end-to-end</name>
  <files>
    <!-- Checkpoint task: no files produced. Human confirms automated work via browser + curl output. -->
  </files>
  <action>Human verification of snippet flow, post flow, browser console, and RBAC — see how-to-verify steps below.</action>
  <verify>All 4 verification steps in how-to-verify pass — snippets render with Shiki, posts save as Draft/Published, zero console errors, RBAC smoke test shows Viewer=403 and Contributor=201.</verify>
...
- Web: 4 post pages (list/new/detail/edit) with textarea+react-markdown split-pane editor; published detail view uses Shiki server-side via MarkdownRenderer
- Production build verified (next build exits 0)
- RBAC smoke tests: Viewer gets 403, Contributor (default role) gets 201
  </what-built>
  <how-to-verify>
...
2. **Post flow** (verify POST-01 through POST-03):
   ...
   - Verify: preview pane shows rendered Markdown with syntax-highlighted code block (react-syntax-highlighter, client-side)
   - Click "Save as Draft" — redirected to post detail, showing "Draft" badge
   - Verify: post detail page shows code fences with Shiki syntax highlighting (server-side, different theme from editor preview)
...
4. **RBAC verification** (verify RBAC-02/RBAC-03):
   - Confirm the curl test output from Task 1 shows: Viewer=403, Contributor=201
   - The Contributor user must have kept the DEFAULT join role (Contributor), not Admin
...
- PostEditor preview pane shows live Markdown rendering with code highlighting (react-syntax-highlighter, client-side)
- Published post detail page renders Shiki-highlighted code server-side via MarkdownRenderer rehype plugin
- Viewer JWT returns 403 on POST /workspaces/:slug/snippets
- Viewer JWT returns 403 on POST /workspaces/:slug/posts
- Contributor JWT (default join role, NOT Admin) returns 201 on POST /workspaces/:slug/snippets
</verification>
...
6. Viewer gets 403 on create snippet/post; Contributor (default role on join) gets 201
</success_criteria>
