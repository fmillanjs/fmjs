...
must_haves:
  truths:
    ...
    - "PATCH /workspaces/:slug/comments/:id allows owner to update content (updatedAt becomes 'edited' indicator)"
  ...
  artifacts:
    - path: "apps/devcollab-api/src/comments/comments.service.ts"
      provides: "create, findAll (flat+tree), update, remove (soft/hard)"
...
<task type="auto">
  ...
  <action>
    ...
    **comments.service.ts** — implement five methods:
    ...
    - `update(slug, id, dto, requesterId)`: workspace lookup → findFirst comment → if comment.authorId !== requesterId, check membership and throw ForbiddenException if not Admin → prisma.comment.update({where:{id}, data:{content:dto.content}})
  ...
  <done>All five comment files compile without TypeScript errors; service implements flat fetch + in-memory tree; soft/hard-delete logic present.</done>
</task>
...
<success_criteria>
All discussion and reaction API endpoints exist and are guarded; flat comment fetch + in-memory tree assembly implemented in CommentsService; soft/hard-delete logic correct; P2002 race condition handled in ReactionsService; TypeScript compiles clean.
</success_criteria>
