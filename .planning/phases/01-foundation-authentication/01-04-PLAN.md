...
must_haves:
  truths:
    ...
    - "Sessions are stored in Redis and persist across browser restarts"
  artifacts:
    - path: "apps/web/lib/auth.ts"
      provides: "NextAuth v5 configuration with Credentials provider and Redis session storage"
      contains: "NextAuth"
  ...
  key_links:
    ...
    - from: "apps/web/lib/auth.ts"
      to: "apps/web/lib/redis.ts"
      via: "Redis adapter for session storage (AUTH-06)"
      pattern: "redis"
...
Purpose: Frontend authentication is the user-facing side of the auth system. Users need to be able to create accounts, log in, and have their sessions persist. This covers AUTH-01 (signup), AUTH-02 (login + session persistence), AUTH-03 (logout), AUTH-06 (secure sessions stored in Redis).
...
<task type="auto">
  <name>Task 1: Configure NextAuth v5 with Credentials provider, Prisma adapter, and Redis session storage</name>
  <files>
    apps/web/package.json
    apps/web/lib/redis.ts
    apps/web/lib/auth.ts
  ...
  <action>
    ...
    ```bash
    npm install next-auth@beta @auth/prisma-adapter @upstash/redis @auth/upstash-redis-adapter bcrypt ioredis --workspace=apps/web
    npm install -D @types/bcrypt --workspace=apps/web
    ```

    **apps/web/lib/redis.ts:**
    Create Redis client for session storage using ioredis connecting to the Docker Redis instance:
    ```typescript
    ...
    Also create an Upstash-compatible Redis wrapper if using @auth/upstash-redis-adapter, OR use @next-auth/redis-adapter with ioredis. The key requirement is that NextAuth stores sessions in Redis (not in JWT cookies or Postgres).

    **Important (AUTH-06):** The session strategy MUST be `'database'`, NOT `'jwt'`. Sessions are stored server-side in Redis, and only a session token cookie is sent to the browser. This satisfies AUTH-06: "User sessions are stored securely in Redis."

    **apps/web/lib/auth.config.ts:**
    Separate auth config for edge runtime (middleware can't use Prisma or Redis):
    ```typescript
    ...
    **apps/web/lib/auth.ts:**
    Full NextAuth v5 config with database session strategy and Redis adapter:
    - PrismaAdapter from @auth/prisma-adapter using prisma from @repo/database (for User/Account storage)
    - **Session strategy: 'database'** (NOT 'jwt') — sessions stored server-side in Redis
    - Use @auth/upstash-redis-adapter or custom Redis adapter wrapping ioredis for session storage. If @auth/upstash-redis-adapter requires Upstash SDK, create a thin adapter wrapper around ioredis that implements the required interface (get, set, del, expire).
    - Session maxAge: 30 days (30 * 24 * 60 * 60 seconds)
    - Credentials provider: validate with loginSchema from @repo/shared, bcrypt compare, return user with id, email, name, role
    - Session callback: add id and role to session.user (read from database user, not JWT token)
    - Extend NextAuth session type with `declare module 'next-auth'` for id and role
    - Export: handlers, auth, signIn, signOut

    **Note on Credentials + database sessions:** NextAuth Credentials provider does not natively create database sessions. You MUST handle session creation manually in the `signIn` callback or use a custom authorize flow that creates the session record in Redis after successful credential validation. One approach: in the `jwt` callback (still needed as intermediate step), persist session data to Redis, and in the `session` callback, read user data from the database. Alternatively, use the `events.signIn` hook to write the session to Redis. Research the latest Auth.js v5 docs for the recommended pattern — the key constraint is that session data ends up in Redis, not solely in a cookie.
    ...
    **apps/web/middleware.ts:**
    ...
    Note: Since this uses authConfig (not full auth.ts), it runs on Edge without Prisma or Redis. The `authorized` callback handles route protection using the session token cookie.
  ...
  <verify>
    ...
    kill %1
    ```
    Verify sessions are stored in Redis (not just JWT cookies):
    ```bash
    docker compose exec redis redis-cli KEYS '*session*'  # Should show session keys after login
    ```
  </verify>
  <done>
    NextAuth v5 configured with Credentials provider and Prisma adapter for user storage. Database session strategy stores sessions in Redis (AUTH-06) with 30-day expiry. Middleware protects dashboard routes and redirects unauthenticated users to /login. Session includes user id and role. Redis contains session records verifiable via redis-cli.
  </done>
</task>
...
<success_criteria>
- NextAuth v5 configured with Credentials provider and Redis-backed database sessions (AUTH-06)
